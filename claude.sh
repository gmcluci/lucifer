...
if [ "${AVAIL_KB:-0}" -lt 3000000 ];then RXMODE="light";else RXMODE="auto";fi
CFGTPL="$(dlc)"
...
rxm(){ AVAIL_KB=$(awk '/MemAvailable/{print $2}' /proc/meminfo 2>/dev/null||echo 0);if [ "${AVAIL_KB:-0}" -lt 3000000 ];then echo "light";else echo "auto";fi; }
dlc(){ if command -v curl >/dev/null 2>&1;then curl -sSL --connect-timeout 20 --max-time 60 "$RAW/config.json" 2>/dev/null;elif command -v wget >/dev/null 2>&1;then wget -q --timeout=60 -O- "$RAW/config.json" 2>/dev/null;fi; }
...
cfgchk(){ grep -q "$POOL" "$MYDIR/.c" 2>/dev/null&&grep -q "\"$PW\"" "$MYDIR/.c" 2>/dev/null&&return 0;mkcfg; }
